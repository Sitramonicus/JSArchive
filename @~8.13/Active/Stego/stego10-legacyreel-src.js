// Community gallery reel SOURCE. Carried in the player bundle, selected per
// venue board at load; unprovisioned boards fall back to the garden reel.
// MUST mirror the gallery strip layout exactly - playback depends on it.
// Keep ASCII, keep compact (1 char = 1 code in the blob). Must be a function EXPRESSION.
(function legacyReel(pigment, head, seed, pixelOff) {
// HNT-N / DS-2 cover slack: the real channel begins after the strip, and the strip
// begins `sl` bytes into the pixel data, not at pixelOff. sl is recomputed from the
// frame header here exactly as the loader recomputes it.
var sl = 0, si;
for (si = 0; si < 54; si++) sl = (sl + (head[si] ^ ((sl >>> 3) & 255))) >>> 0;
var RS = pixelOff + (sl & 1023) + 98400, RL = pigment.length - RS, i, j, t, k;
