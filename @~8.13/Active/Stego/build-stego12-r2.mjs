...
import { PIXOFF, STRIP_LEN, embedReal, embedDecoy, dseedFor, slackFor, stripOff, rStartFor } from './stego3-codec.mjs';
...
(async () => {
  ...
  // Pin what ships (same model as V2 pins). Bug: tG7HexxSM live log 2026-09-14.
  {
    ...
    const _noisePath = path.join(__dirname, '..', 'O8.13', 'oto', 'grep-noise.json');
    if (fs.existsSync(_noisePath)) {
      ...
      if (Array.isArray(_items) && _items.length) {
        ...
        minReal = minReal.replace(/;?\s*$/, '') + _expr + ';';
        console.log(`[HNT-GREP] re-appended ${_items.length} noise strings after terser (+${_expr.length} B)`);
      }
  ...
  const nR = embedReal(stego, shead, seed, realPayload);
  ...
  console.log(`[+] cover slack ${SLACK} B -> strip @${S0}, real @${S0 + STRIP_LEN} (loader + reels recompute from the header)`);
