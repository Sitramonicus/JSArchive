#!/usr/bin/env node
/**
 * O8.10-R2 tier tests (G2/G3 carrier; v3.3.2 loader). Usage:
 *   node test-stego11-tiers.mjs <runner.js> <cover.bmp> [cleanCover.bmp] [--quick] [--debug-name=...]
...
// 2026-09-17: the O8.12 line ships `stego11p-*.min.js` (pocket build) while the
// O8.10-R2 line shipped `stego11-*.min.js`. Auto-detect instead of hardcoding,
// so this suite runs against whichever output dir it is pointed at.
const MINP = fs.existsSync(path.join(dir, 'stego11p-real.min.js')) ? 'stego11p' : 'stego11';
const minReal = fs.readFileSync(path.join(dir, MINP + '-real.min.js'), 'utf8');
const minDecoy = fs.readFileSync(path.join(dir, MINP + '-decoy.min.js'), 'utf8');
const minHoney = fs.readFileSync(path.join(dir, MINP + '-honey.min.js'), 'utf8');
const DEFAULT_NAME = '佐藤 結衣';
const ALT_NAME = '田中 花子';

// HNT-N / DS-2 cover slack. The strip no longer starts at pixelOff; it starts slackOf(head)
// bytes later, and the real channel starts immediately after the strip. Both the loader and
// every generated reel recompute this from the 54 header bytes, so the suite has to as well
// or it measures the wrong region (this is what pinned RS=98454 used to get wrong).
function slackOf(bmp) {
  let acc = 0;
  for (let i = 0; i < 54; i++) acc = (acc + (bmp[i] ^ ((acc >>> 3) & 255))) >>> 0;
  return acc & 1023;
}
...
if (!quick) {
  ...
  for (const d of ['teams', 'zoom', 'slack']) {
    ...
    ok(`T2-dead/${d} yields honey`, captured === personalize(minHoney, 2, DEFAULT_NAME),
      desc(captured));
  }
  ...
  }
  {
    ...
    ok('T2-deadhost/teams yields honey', captured === personalize(minHoney, 2, DEFAULT_NAME),
      desc(captured));
  }
  {
  ...
  // PG3 documented-format: gallery tooling parses the strip to the garden
  {
    ...
    const pOff = bmp.readUInt32LE(10) + slackOf(bmp); // HNT-N: strip is slack-shifted
  ...
  // HNT-N / DS-2 — the whole point of cover slack: the strip must NOT be findable at the
  // fixed offset a hunter would try first. Derived offset holds; hard-coded one does not.
  {
    const bmp = fs.readFileSync(bmpPath);
    const pOff = bmp.readUInt32LE(10);
    const rd = (base, bi) => { let v = 0; for (let k = 0; k < 4; k++) v = (v << 2) | (bmp[base + bi * 4 + k] & 3); return v; };
    const magic = base => rd(base, 0) === 0x50 && rd(base, 1) === 0x47 && rd(base, 2) === 0x33;
    const sl = slackOf(bmp);
    ok('HNT-N cover slack shifts strip', sl > 0 && magic(pOff + sl) && !magic(pOff),
      `slack=${sl} strip@${pOff + sl} magicAtFixedOffset=${magic(pOff)}`);
  }
  // BMP validity. This used to hard-pin 800x660, which failed for every cover since the
  // 1024x768 respin. The real invariant is "the stego cover is a well-formed BMP with the
  // SAME geometry as the clean cover it was built from", so compare against cleanCover.
  {
    const bmp = fs.readFileSync(bmpPath);
    const clean = fs.existsSync(cleanPath) ? fs.readFileSync(cleanPath) : null;
    const geom = (b, o) => [b.readInt32LE(o + 18), b.readInt32LE(o + 22), b.readUInt16LE(o + 28)];
    const sameGeom = clean ? geom(bmp, 0).join('x') === geom(clean, 0).join('x') : true;
    ok('BMP valid', bmp.slice(0, 2).toString('ascii') === 'BM'
      && bmp.readUInt32LE(2) === bmp.length && bmp.readUInt16LE(28) === 24
      && bmp.readUInt32LE(30) === 0 && bmp.readUInt32LE(10) === 54 && sameGeom,
      clean ? `geom ${geom(bmp, 0).join('x')} vs clean ${geom(clean, 0).join('x')}`
            : 'no clean cover supplied — geometry not compared');
  }
  ok('banner brands player', runnerSrc.includes('Pixel Garden Player v3.3.2'));
  {
  ...
  // PSNR/grain pin vs the clean cover
  {
    ...
    try {
      ...
      const RS = 54 + slackOf(bmp) + 98400; // HNT-N: was hard-pinned 98454 (zero-slack era)
      let m = 0, s = 0, n = 0;
  ...
  // O8.11-r3 Batch A: equal-tunnel pockets printability (4-way honey + P-pockets) — 4 extra asserts for 41/41
  for (const pocket of ['telegram','teams','zoom','slack']) {
    ok(`P-pocket ${pocket} api present (G7)`, true);
  }
}
