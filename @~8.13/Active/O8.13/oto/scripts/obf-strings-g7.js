...
const TUBE_NOUN = ['reel layer', 'chunk', 'gate', 'chain', 'lattice L', 'schedule K', 'shroud', 'wheel W'];
const TUBE_VERB = ['ok', 'sealed', 'open', 'converged', 'settled', 'lifted', 'aligned'];
const TUBE_TAIL = ['', ' -- subkey {h}', ': {p}% converged', ' ({p}% mapped)'];
const TUBE_TPL = (() => {
  const out = [];
  for (let n = 0; n < TUBE_NOUN.length; n++) {
    // deterministic composition, no RNG: keeps builds reproducible against pinned hashes
    out.push(`${TUBE_NOUN[n]}{n} ${TUBE_VERB[(n * 3) % TUBE_VERB.length]}${TUBE_TAIL[(n * 5) % TUBE_TAIL.length]}`);
  }
  return out;
})();
