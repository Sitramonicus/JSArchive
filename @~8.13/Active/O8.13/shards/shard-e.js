  ...
  (async () => {
    try {
      ...
      const _0xvmExec = (prog, vars) => {
        const st = [], vs = Object.assign({}, vars), table = Object.create(null);
        const k = _0xruntimeKeyReady ? _0xruntimeKey : 0x6d7f0c87;
        const slot = op => ((op ^ (k & 255)) + ((k >>> 8) & 31)) & 255;
        const bind = (op, fn) => { table[slot(op)] = fn; };
        bind(0x01, (pc) => st.push(prog[pc + 1]));
        bind(0x02, (pc) => st.push(vs[prog[pc + 1]]));
        bind(0x03, () => { const b = st.pop(), a = st.pop(); st.push(a + b); });
        bind(0x04, () => { const b = st.pop(), a = st.pop(); st.push(a - b); });
        bind(0x05, () => { const b = st.pop(), a = st.pop(); st.push(a * b); });
        bind(0x09, (pc) => { vs[prog[pc + 1]] = st.pop(); });
        bind(0x0D, () => { const b = st.pop(), a = st.pop(); st.push(Math.max(a, b)); });
        bind(0x0F, () => st.pop());
        for (let pc = 0; pc < prog.length;) {
          const op = prog[pc], fn = table[slot(op)];
          if (typeof fn !== 'function') throw new Error('vm-op');
          const before = st.length, out = fn(pc);
          // Mutate an occupied dispatch slot without changing semantics; this keeps
          // the core's operation order dependent on the runtime key.
          if ((pc & 3) === 0) table[slot(op)] = fn;
          if (op === 0x01 || op === 0x02 || op === 0x09) pc += 2; else pc += 1;
          if (op === 0x0F) return out;
          if (st.length < before - 2) throw new Error('vm-stack');
        }
        return st.pop();
      };
      ...
      const _0xverifyDone = (v, cleanup) => { // silence backstop: play/stream are event-only; verify completion via store
        try {
          ...
          if (_0vdone) { try { Log.diag('Completion verified', { chore: v.name, via: _0vvia }); } catch (e) {} cleanup(); }
        } catch (e) {}
      };
      ...
      const _0xlatencyFaC = async () => {
        ...
        try { Log.diag('Store timing profile', { elapsedMs: elapsed, plausible, shape: shape.length }); } catch (e) {}
        return plausible;
      };
      const _0x2c = async () => {
        ...
        const _0xrefill = () => {
          try {
            ...
            if (_0storeDead) {
              ...
              try { Log.diag("Queue refill evaluation", { added: 0, storeDead: true, totalCandidateStores: -1, recheckWindowSeconds: 15 }); } catch (e) {}
              return 0;
            }
            let _0added = 0;
            ...
            Log.diag("Queue refill evaluation", { added: _0added, totalCandidateStores: _0list.length, skipped: _0skip, recheckWindowSeconds: 15 });
            try {
