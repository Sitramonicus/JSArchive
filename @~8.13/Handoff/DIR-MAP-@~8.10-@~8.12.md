...
## Why version folders & @~8.13 Structure
...
- `@~8.12` holds the **RIPCORD line** (O8.12-r4 `1e03f48`).
- `@~8.13` holds the **O8.13 active line & bench**:
  - `Working-Stable/O8.13/` (the live shippable deliverable `O8.13-runner-e008b377.js`)
  - `Active/O8.13/` & `Active/Stego/` (active buildbench)
  - `Archives/packages/` (frozen package line mirrors)
  - `Handoff/` (living documentation and running changelog)
  - **`8.13-SF/` (`[<repo>/@~8.13/8.13-SF]`):** Staging container for saved / superfluous historical files (~50 MB) moved out of the active sandbox to preserve headroom (~52.8 MB clean sandbox). Holds `_COMPRESSED-HISTORY/`, `_BACKUPS/`, `Docs/`, `shards-history/`, `@~8.12/`, `WORKSPACE-STATUS.md`, `README.md.old`, etc.
  - **Removal rule:** Once committed to the GitHub repo under `@~8.13/8.13-SF`, this folder can be deleted from the sandbox root (`rm -rf 8.13-SF`) while retaining context in the remote repository. Future agents access it via the GitHub repo.

**Freeze rule (unchanged):** overwriting `Archives/packages/<line>/` etc. Now means `@~8.13/Archives/packages/<line>/`.
