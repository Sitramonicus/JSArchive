...
> **2026-09-17 UPDATE — read `Handoff/SESSION-2026-09-17-NIL-RESUME.md` first.** This file's claims that
> `O8.12-r4` is `S15 ALL PASS` and that hard-coded `O8.6` refs were updated are **both wrong**. The battery is
> now a genuine 25/25 after path fixes in `O8.13/`; the S15 tier suite is **28 passed / 12 failed** on the live
> pair. Snapshot is now **119.4 MB** after the `@~8.10` archive + dedup described in that session note.
