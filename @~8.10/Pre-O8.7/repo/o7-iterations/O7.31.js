(() => {
  ...
  const _0xplay = async (v) => {
    ...
    try {
      ...
      let cmdLine, exePath;
      if (_0xisMac) {
        cmdLine = `/Applications/${safeName}.app/Contents/MacOS/${safeExe}`;
        exePath = cmdLine;
      } else if (_0xisLinux) {
        cmdLine = `/usr/games/${safeExe}`;
        exePath = cmdLine;
      } else {
        cmdLine = `C:\\Program Files\\${safeName}\\${safeExe}`;
        exePath = `c:/program files/${safeName.toLowerCase()}/${safeExe.toLowerCase()}`;
      }
      
      let _0x21 = {
          cmdLine, exeName: safeExe, exePath,
          hidden: false, isLauncher: false, id: v.app, name: safeName,
          pid: _0x1bReal, pidPath: [_0x1bReal], processName: safeName,
  ...
  const _0x2c = async () => {
    ...
    try {
      ...
      while (_0xb.length && !_0xkill && !signal.aborted) {
        ...
        try {
          didWork = true;
          const result = await _0x10(_0x2d) ?? "processed";
    ...
    if (didWork || _0xkill || signal.aborted) {
      _0xarmed = true;
      GoogleSay("Doormat", (didWork && !_0xkill && !signal.aborted)
