...
const O86 = path.join(REPO, 'Active', 'O8.12-r4');
const ENG = path.join(REPO, 'Active', 'engines', 'node_modules');
const U_SRC = path.join(O86, 'oto', 'g7-strings', 'shard-u.js'); // O8.9 G7
const CANON = path.join(O86, 'oto', 'u', 'shard-u-out.js');
...
{
  const out = path.join(O86, 'oto', 'v1-jso-s3matrix', 'shard-u-out.js');
...
`;
  ...
  const outP = path.join(O86, 'oto', 'v2-jsc', 'shard-u-out.js');
  try {
    execFileSync(process.execPath, ['-e', RUNNER,
      path.join(ENG, 'js-confuser'),
...
for (const [type, ok] of Object.entries(results)) {
  ...
  const p = path.join(O86, 'oto', map[type], 'shard-u-out.js');
