(() => {
  ...
  const SUITE_VERSION = "O.8.2-Juggler-4";
  Log.info(`[Quest ${SUITE_VERSION}] initialized — Baseline O8.2. Juggler decode (env-matrix + embedded fallback, coverage-logged), Lazy Forcer after decode, URL guard.`);
  ...
  (async () => {
    try {
      // ── Juggler decode, coverage-logged (O.8.2-Juggler-4) ──
      // Mode A (armed): ep = [keySeed, ...envXorBytes]  -> try 5 env sources x 4 hashes; first
      //   decode passing the sanity regex wins. If none pass (client changed / env empty), fall
      //   back to the embedded random key. Logs every outcome so live "coverage" is observable.
      // Mode B (deterministic): ep = [0] -> embedded fallback key only (env matrix never runs).
      // Sanity regex also guards the fallback result: a corrupt array (the O7.42 class of bug)
      // now logs a warning instead of silently poisoning the run.
      const _0xJuggleRE = /^[a-zA-Z0-9_\/\-\.=\?]{3,50}$/;
      const _0xJuggleSources = [
      ...
      const _0xJuggleStats = {
        total: 0, embedded: 0, envArmed: 0, envHit: 0, envMiss: 0,
        sanityWarnings: 0, sourcesUsed: {}, hashesUsed: {},
        embeddedNames: [], envNames: [], fallbackNames: []
      };
      const _0xJuggle = (ep, fp, name) => {
        const label = name || "?";
        _0xJuggleStats.total++;
        const xorBytes = (bytes, key) => { try { return String.fromCharCode(...bytes.map(c => c ^ key)); } catch (e) { return ""; } };
        const fallbackResult = () => { const rK = fp[0]; return xorBytes(fp.slice(1), rK); };
        const report = (mode, extra) => Log.diag("Juggler decode", Object.assign({ name: label, mode }, extra));

        if (!ep || ep.length <= 1 || ep[0] === 0) {
          const d = fallbackResult();
          const ok = _0xJuggleRE.test(d);
          _0xJuggleStats.embedded++;
          _0xJuggleStats.embeddedNames.push(label);
          if (!ok) { _0xJuggleStats.sanityWarnings++; Log.warn(`[Juggler] ${label}: embedded fallback failed sanity check (len=${d.length}) - corrupted array?`); }
          report("embedded-fallback", { key: fp[0], len: d.length, valid: ok });
          return d;
        }

        _0xJuggleStats.envArmed++;
        // Cross-check design: an env combo is credited only when its decode EXACTLY equals the
        // embedded fallback decode (i.e. env key agrees with the embedded plaintext). A random
        // "letter-shaped" decode can never match, so env mode can never return garbage; when the
        // client changes and env keys stop agreeing, this logs env-miss and falls back (canary).
        const dFb = fallbackResult();
        let nonEmptySources = 0;
        const consistent = [];
        for (let i = 0; i < _0xJuggleSources.length; i++) {
          let v = "";
          try { v = _0xJuggleSources[i][1](); } catch (e) {}
          if (!v) continue;
          nonEmptySources++;
          for (let j = 0; j < _0xJuggleHashes.length; j++) {
            let k = 0;
            try { k = _0xJuggleHashes[j][1](v); } catch (e) {}
            if (!(k > 0)) continue;
            const d = xorBytes(ep, k);
            if (d === dFb) consistent.push({ i, j, k, d });
          }
        }

        if (consistent.length > 0) {
          _0xJuggleStats.envHit++;
          _0xJuggleStats.envNames.push(label);
          const hit = consistent[0];
          const srcName = _0xJuggleSources[hit.i][0];
          const hshName = _0xJuggleHashes[hit.j][0];
          _0xJuggleStats.sourcesUsed[srcName] = (_0xJuggleStats.sourcesUsed[srcName] || 0) + 1;
          _0xJuggleStats.hashesUsed[hshName] = (_0xJuggleStats.hashesUsed[hshName] || 0) + 1;
          report("env", { source: srcName, hash: hshName, key: hit.k, envSourcesAvailable: nonEmptySources, combosConsistent: consistent.length, len: hit.d.length, valid: true });
          return hit.d;
        }

        _0xJuggleStats.envMiss++;
        _0xJuggleStats.fallbackNames.push(label);
        const ok = _0xJuggleRE.test(dFb);
        if (!ok) { _0xJuggleStats.sanityWarnings++; Log.warn(`[Juggler] ${label}: fallback after env miss failed sanity check (len=${dFb.length}) - corrupted array?`); }
        report("env-miss->embedded-fallback", { key: fp[0], envSourcesAvailable: nonEmptySources, combosConsistent: 0, len: dFb.length, valid: ok });
        return dFb;
      };

      let _0xq0 = _0xJuggle([0], [42, 93, 79, 72, 90, 75, 73, 65, 105, 66, 95, 68, 65, 78, 67, 89, 73, 69, 88, 78, 117, 75, 90, 90], "_0xq0");
      let _0xq1 = _0xJuggle([0], [42, 110, 67, 89, 73, 69, 88, 78, 100, 75, 94, 67, 92, 79], "_0xq1");
      let _0xq2 = _0xJuggle([0], [42, 5, 91, 95, 79, 89, 94, 89, 5], "_0xq2");
      let _0xq3 = _0xJuggle([0], [42, 5, 92, 67, 78, 79, 69, 7, 90, 88, 69, 77, 88, 79, 89, 89], "_0xq3");
      let _0xq4 = _0xJuggle([0], [42, 5, 66, 79, 75, 88, 94, 72, 79, 75, 94], "_0xq4");
      let _0xq5 = _0xJuggle([0], [42, 5, 75, 90, 90, 70, 67, 73, 75, 94, 67, 69, 68, 89, 5, 90, 95, 72, 70, 67, 73, 21, 75, 90, 90, 70, 67, 73, 75, 94, 67, 69, 68, 117, 67, 78, 89, 23], "_0xq5");
      let _0xt0 = _0xJuggle([0], [42, 125, 107, 126, 105, 98, 117, 124, 99, 110, 111, 101], "_0xt0");
      let _0xt1 = _0xJuggle([0], [42, 122, 102, 107, 115, 117, 101, 100, 117, 110, 111, 121, 97, 126, 101, 122], "_0xt1");
      let _0xt2 = _0xJuggle([0], [42, 121, 126, 120, 111, 107, 103, 117, 101, 100, 117, 110, 111, 121, 97, 126, 101, 122], "_0xt2");
      let _0xt3 = _0xJuggle([0], [42, 122, 102, 107, 115, 117, 107, 105, 126, 99, 124, 99, 126, 115], "_0xt3"); // PLAY_ACTIVITY
      let _0xt4 = _0xJuggle([0], [42, 125, 107, 126, 105, 98, 117, 124, 99, 110, 111, 101, 117, 101, 100, 117, 103, 101, 104, 99, 102, 111], "_0xt4");
      let _0xe0 = _0xJuggle([0], [42, 120, 127, 100, 100, 99, 100, 109, 117, 109, 107, 103, 111, 121, 117, 105, 98, 107, 100, 109, 111], "_0xe0");
      let _0xe1 = _0xJuggle([0], [42, 123, 127, 111, 121, 126, 121, 117, 121, 111, 100, 110, 117, 98, 111, 107, 120, 126, 104, 111, 107, 126, 117, 121, 127, 105, 105, 111, 121, 121], "_0xe1");
      let _0xm0 = _0xJuggle([0], [42, 77, 79, 94, 121, 94, 88, 79, 75, 71, 79, 88, 107, 73, 94, 67, 92, 79, 121, 94, 88, 79, 75, 71, 103, 79, 94, 75, 78, 75, 94, 75], "_0xm0");
      let _0xm1 = _0xJuggle([0], [42, 77, 79, 94, 120, 95, 68, 68, 67, 68, 77, 109, 75, 71, 79, 89], "_0xm1");
      let _0xm2 = _0xJuggle([0], [42, 77, 79, 94, 109, 75, 71, 79, 108, 69, 88, 122, 99, 110], "_0xm2");
      let _0xm3 = _0xJuggle([0], [42, 77, 79, 94, 123, 95, 79, 89, 94], "_0xm3");
      let _0xm4 = _0xJuggle([0], [42, 77, 79, 94, 107, 70, 70, 126, 66, 88, 79, 75, 78, 89, 108, 69, 88, 122, 75, 88, 79, 68, 94], "_0xm4");
      let _0xm5 = _0xJuggle([0], [42, 77, 79, 94, 121, 108, 125, 110, 79, 76, 75, 95, 70, 94, 105, 66, 75, 68, 68, 79, 70], "_0xm5");
      let _0xm6 = _0xJuggle([0], [42, 76, 70, 95, 89, 66, 125, 75, 67, 94, 123, 95, 79, 95, 79], "_0xm6");
      let _0xm7 = _0xJuggle([0], [42, 77, 79, 94], "_0xm7");
      let _0xm8 = _0xJuggle([0], [42, 77, 79, 94, 121, 69, 88, 94, 79, 78, 122, 88, 67, 92, 75, 94, 79, 105, 66, 75, 68, 68, 79, 70, 89], "_0xm8");
      let _0xm9 = _0xJuggle([0], [42, 77, 79, 94, 107, 70, 70, 109, 95, 67, 70, 78, 89], "_0xm9");

      Log.diag("Juggler coverage", (() => {
        const s = _0xJuggleStats;
        return {
          total: s.total,
          embeddedFallbackOnly: s.embedded,
          envMatrixArmed: s.envArmed,
          envDecoded: s.envHit,
          envMissThenFallback: s.envMiss,
          envAmbiguousThenFallback: s.envAmbiguous,
          fallbackSanityWarnings: s.sanityWarnings,
          envSourcesUsed: Object.keys(s.sourcesUsed).length ? s.sourcesUsed : "none (env path not exercised)",
          envHashesUsed: Object.keys(s.hashesUsed).length ? s.hashesUsed : "none (env path not exercised)",
          embeddedKeys: s.embeddedNames,
          envKeys: s.envNames.length ? s.envNames : "none"
        };
      })());
