(() => {
  ...
  const SUITE_VERSION = "O.8.2-Juggle-diag";
  ...
  (async () => {
    try {
      ...
      // [O.8.2-Juggle-fix] URL-id hardening (single-subsystem revision).
      // Some live quest configs deliver the application/quest id wrapped in an object;
      // concatenating that object into a URL serializes it as "[object Object]" (the
      // observed 404 on https://discord.com/[object%20Object]). Recover a string id when
      // the wrapper exposes one, else fall back to String() and log the shape at diag level.
      const GoogleId = (v, what) => {
        ...
        let shape = "?";
        try { shape = v === null ? "null" : Array.isArray(v) ? "array" : typeof v; } catch (e) {}
        let json = "?";
        try { json = JSON.stringify(v); } catch (e) { try { json = String(v); } catch (x) { json = "<unstringifiable>"; } }
        Log.diag("URL id non-string", { what, type: shape, sample: json });
        return String(v ?? "");
      };
        ...
        while (tries < 3 && !signal.aborted) {
          try {
            const finalOpts = (opts && typeof opts.url !== "string")
              ? (Log.diag("Call received non-string url", { type: typeof opts.url, sample: (() => { try { return JSON.stringify(opts.url); } catch (e) { return String(opts.url); } })() }), { ...opts, url: String(opts.url ?? "") })
              : opts;
            Log.diag("HTTP url", { url: String(finalOpts?.url ?? "") });
