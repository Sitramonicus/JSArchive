...
(() => {
  ...
  const Log = (() => {
    ...
    return {
      ...
      const _0xsalt = String.fromCharCode(...[116,104,105,115,105,115,106,117,115,116,102,111,114,100,101,98,117,103,103,105,110,103,119,104,121,105,110,116,104,101,104,101,108,108,100,111,121,111,117,110,101,101,100,116,111,107,110,111,119,116,104,101,99,111,110,116,101,110,116,115]);
      const _0xwant = "3d8c3100982128c46c3b5bed6be85b1da501adb14c014244baae5d61386a6a71";
  ...
  Log.diag(`O8.4.1 core — static lexicon, log-lock, studio gaps, refill queue, metamorphic decode.`);
  ...
  (async () => {
    try {
      ...
      if (!_0x1 || typeof _0x1.push !== "function" || typeof _0x1.pop !== "function") {
        Log.say("Crumb", "The module doorway is unavailable — heading home.");
        GoogleRelease();
        return;
      }
      ...
      if (!_0x2 || typeof _0x2.c !== "object") {
        Log.say("Crumb", "The module doorway returned no usable runtime — heading home.");
        GoogleRelease();
        return;
      }
      ...
      if (
        ...
        Log.say("Crumb", "Pocket interfaces invalid — heading home.");
        GoogleRelease();
        return;
      }
      ...
      const _0x44c1 = (cfg, supportedTasks) => {
      ...
      let _0xb = _0xeligible.filter(q => {
        const tasks = _0x44c1(q.config, GoogleRoutes.tasks)?.tasks;
        return tasks;
      });
      ...
      Log.say("Docket", `${_0xb.length} chore${_0xb.length === 1 ? "" : "s"} pinned to the board.`);
      const _0xlost = _0xeligible.length - _0xb.length;
      if (_0xlost > 0) Log.say("Docket", `${_0xlost} left off — shape we can't fold.`);
      
      if (!_0xb.length) { 
        Log.say("Greenhouse", "Nothing ripe on the trees today. (Press Alt+Shift+R to flush and restart)"); 
        GoogleRelease(); 
        return; 
      }
      ...
      const _0xe63b = (ms, sig) => new Promise((res, rej) => {
      ...
      let GoogleDelay = async (d = 1) => {
        if (document.hidden !== _0xlastHidden) {
          ...
          Log.say("Drapes", document.hidden ? "Drapes drawn — taking the long hallway." : "Drapes open — back on the main road.");
        }
        ...
        while (remaining > 0 && !signal.aborted) {
          if (_0xpaus) { await _0xe63b(900, signal); continue; }
          const chunk = Math.min(remaining, 5000);
          const start = Date.now();
          try { await _0xe63b(chunk, signal); } catch(e) { if(e.name === 'AbortError') return; throw e; }
      ...
      _0xchord = (e) => {
        ...
        if (key === 'x' && !_0xkill && !_0xarmed) { _0xkill = true; controller.abort(); Log.say("Chime", "Wrapping up after this chore."); }
        if (key === 'r' && _0xarmed) { Log.say("Wreath", "Shaking out the rug — see you on the other side."); GoogleRelease(); setTimeout(() => location.reload(), 1500); }
      };
      document.addEventListener("keydown", _0xchord, true);
      _0xbe2f(() => document.removeEventListener("keydown", _0xchord, true));
      ...
      const GoogleCall = (fn, critical = false) => async (opts) => {
        ...
        while (tries < 3 && !signal.aborted) {
          try {
            ...
            if (st === 401) { if (critical) { _0xkill = true; controller.abort(); Log.say("Crumb", "Key stopped fitting — packing up."); } throw e; }
            if (st === 429) {
              ...
              Log.say("Veranda", `Knock came back throttled — knocking again in ~${Math.ceil(s)}s.`);
              await GoogleDelayRaw(s * 1000); tries++; continue;
            }
            if (st >= 500 && st < 600) { 
              const backoff = Math.pow(2, tries) * 2 + (Math.random() * 2); 
              Log.say("Veranda", `Server error ${st} — backing off for ${backoff.toFixed(1)}s.`); 
      ...
      const GoogleHook = (obj, key, fn) => {
        if (!obj || Object.isFrozen(obj) || Object.isSealed(obj)) { Log.say("Crumb", `Target ${key} is immutable — hook failed.`); return null; }
        try {
        ...
        } catch (e) { Log.say("Crumb", `Hook installation failed for ${key}: ${e.message}`); return null; }
      };
      ...
      const _0x1787 = (str) => String(str || "").replace(/[\/\\:*?"<>|]/g, "");

      const _0xvideo = async (v) => {
        Log.say("Storybook", `Opening the storybook for ${v.name}.`);
        let tick = 0, lastTs = 0;
        while (v.cur < v.goal && !_0xkill && !signal.aborted) {
          ...
          if (Math.random() < 0.06) { Log.say("Whistle", "Whistle's on — brief steep."); await GoogleDelay(18 + Math.random() * 24); if (_0xkill || signal.aborted) break; }
          ...
          if (tick === 0) Log.say("Chronometer", `Sands landing whole: ${Number.isInteger(ts)} — sand #${ts}`);
          let _0x1d = await GooglePostSafe({ url: GoogleRoutes.videoProgress(v.q.id), body: { timestamp: ts } });
          const reported = _0x2280(_0x1d?.body, v.taskType);
          if (reported !== null) v.cur = reported; else v.cur = Math.min(v.goal, v.cur + _0x1c);
          if (++tick % 3 === 1 || v.cur >= v.goal) Log.say("Sundial", `Random fraction: ${v.cur.toFixed(2)}/${v.goal}`);
          if (_0x1d?.body?.completed_at) break;
        }
        if (v.cur >= v.goal) Log.say("Garland", `Polished: ${v.name}.`);
      };

      const _0xplay = async (v) => {
        const taskId = Symbol(); _0xcc13.add(taskId);
        let taskFinished = false; 
        const finishTask = () => { if (!taskFinished) { taskFinished = true; _0xcc13.delete(taskId); } };
        
        return new Promise((resolve, reject) => {
          (async () => {
            let handedOff = false;
            try {
              if (!_0xc) { Log.say("Crumb", `That chore (${v.name}) needs the big workshop — skipping.`); resolve(); return; }
              ...
              let _0x1f = _0x1e?.body?.[0]; if (!_0x1f) { Log.say("Crumb", "Chore note came back blank — skipping."); resolve(); return; }
              ...
              Log.say("Mosaic", `Floor mosaic lines up in fours: ${_0x1bReal % 4 === 0} — tile #${_0x1bReal}`);
              const safeName = _0x1787(_0x1f.name); const safeExe = _0x1787(_0x20);
              let cmdLine, exePath;
              ...
              Log.say("Flatware", "Flatware drawer reshuffled: " + Object.keys(_0x21).join(", "));
              ...
              } catch (e) { try { if (typeof undo2 === 'function') undo2(); } catch (x) {} try { if (typeof undo1 === 'function') undo1(); } catch (x) {} Log.say("Crumb", "The desktop doorway could not be prepared — skipping."); resolve(); return; }
              ...
              const cleanup = () => { 
                ...
                finishTask();
                Log.diag("Desktop task cleanup", { state: "cleaned", activeTaskCount: _0xcc13.size }); 
              ...
              removeSelf = _0xbe2f(cleanup);
              
              try { _0xsend({ type: _0xe0, removed: running, added: [_0x21], games: _0x23 }); } catch (e) { cleanup(); Log.say("Crumb", "Initial state dispatch failed — skipping."); resolve(); return; }
              if (cleanupCalled || signal.aborted || _0xrunOwner.released) { resolve(); return; }
              
              let stick = 0;
              GoogleDesktopHandler = data => { if (_0xpaus || _0xkill || signal.aborted || cleanupCalled) return; let _0x26 = GoogleProgress(data, GoogleTasks.play, v.cfgv); if (_0x26 === null) return; if (++stick % 3 === 1 || _0x26 >= v.goal) Log.say("Sundial", `Random fraction: ${_0x26}/${v.goal}`); if (_0x26 >= v.goal) Log.say("Garland", `Polished: ${v.name}.`); if (_0x26 >= v.goal || _0xkill) cleanup(); };
              watchdog = setTimeout(() => cleanup(), Math.max(10 * 60 * 1000, (v.goal - v.cur) * 120000));
              
              try { _0xon(_0xe1, GoogleDesktopHandler); } catch (e) { cleanup(); Log.say("Crumb", "Desktop progress subscription failed — skipping."); resolve(); return; }
              
              // [RESTORED] Handoff diagnostic
              Log.diag("Desktop task handoff", { state: "subscribed", activeTaskCount: _0xcc13.size });
              handedOff = true; 
              Log.say("Scraps", `Mulling over notes for ${safeName} — dough needs ~${Math.ceil((v.goal - v.cur) / 60)} more minutes.`);
            } catch (err) {
              finishTask();
              reject(err);
            } finally {
        ...
        return new Promise((resolve, reject) => {
          (async () => {
            ...
            try {
              ...
              Log.say("Mosaic", `Floor mosaic lines up in fours: ${_0xpid % 4 === 0} — tile #${_0xpid}`);
              let undo = null;
              try { undo = GoogleHook(_0x3, _0xm0, GoogleNative(() => ({ id: v.app, pid: _0xpid, sourceName: null }), "function getStreamerActiveStreamMetadata() { [native code] }", _0xm0, 0)); if (!undo) throw new Error("hook"); } catch (e) { Log.say("Crumb", "The stream doorway could not be prepared — skipping."); resolve(); return; }
              ...
              const cleanup = () => { 
                ...
                // [RESTORED] Cleanup diagnostic
                finishTask();
                Log.diag("Stream task cleanup", { state: "cleaned", activeTaskCount: _0xcc13.size }); 
                if (removeSelf) removeSelf(); 
                try { undo?.(); } catch (e) {} 
              ...
              let stick = 0;
              GoogleStreamHandler = data => { if (_0xpaus || _0xkill || signal.aborted || cleanupCalled) return; let _0x28 = GoogleProgress(data, GoogleTasks.stream, v.cfgv); if (_0x28 === null) return; if (++stick % 3 === 1 || _0x28 >= v.goal) Log.say("Sundial", `Random fraction: ${_0x28}/${v.goal}`); if (_0x28 >= v.goal) Log.say("Garland", `Polished: ${v.name}.`); if (_0x28 >= v.goal || _0xkill) cleanup(); };
              watchdog = setTimeout(() => cleanup(), Math.max(10 * 60 * 1000, (v.goal - v.cur) * 120000));
              
              try { _0xon(_0xe1, GoogleStreamHandler); } catch (e) { cleanup(); Log.say("Crumb", "Stream progress subscription failed — skipping."); resolve(); return; }
              
              // [RESTORED] Handoff diagnostic
              Log.diag("Stream task handoff", { state: "subscribed", activeTaskCount: _0xcc13.size });
              handedOff = true; 
              Log.say("Marquee", `Backstage lights on — keep a window live in vc for ~${Math.ceil((v.goal - v.cur) / 60)} more minutes.`);
            } catch (err) {
              finishTask();
              reject(err);
            } finally {
      ...
      const _0xact = async (v) => {
        if (!_0x6 || !_0x7) { Log.say("Crumb", "No doorway found for the arcade cabinet — skipping."); return; }
        let _0x29; try { _0x29 = _0x6[_0xm8]()[0]?.id; } catch (e) {}
        if (!_0x29) { try { const guilds = Object.values(_0x7[_0xm9]()); const voiceGuild = guilds.find(x => x && x.VOCAL && x.VOCAL.length); if (voiceGuild) _0x29 = voiceGuild.VOCAL[0].channel.id; } catch (e) {} if (!_0x29) { Log.say("Crumb", "No doorway found for the arcade cabinet — skipping."); return; } }
        let _0x2a = "call:" + _0x29 + ":" + Array.from({length: 16}, () => Math.floor(Math.random() * 16).toString(16)).join('');
        Log.say("Fairground", `Feeding coins to the cabinet (~${Math.ceil((v.goal - v.cur) / 60)} min).`);
        ...
        while (v.cur < v.goal && !_0xkill && !signal.aborted && Date.now() < _0xactivityDeadline) {
          ...
          const reportedProgress = _0x2280(_0x2b?.body, GoogleTasks.activity);
          if (reportedProgress !== null) v.cur = reportedProgress;
          if (++tick % 3 === 1 || v.cur >= v.goal) Log.say("Sundial", `Random fraction: ${v.cur}/${v.goal}`);
        ...
        if (v.cur >= v.goal) Log.say("Garland", `Polished: ${v.name}.`);
        else if (!_0xkill && !signal.aborted && Date.now() >= _0xactivityDeadline) Log.say("Crumb", `Activity ${v.name} stopped after no confirmed progress.`);
      };
      ...
      const _0x10 = async (_0x11) => {
        let _0x15 = _0x44c1(_0x11.config, GoogleRoutes.tasks);
        if (!_0x15?.tasks) { Log.say("Crumb", "Chore list was blank — skipping this one."); return "skipped"; }
        ...
        if (!Number.isFinite(_0x19) || _0x19 <= 0) { Log.say("Crumb", "Chore target was invalid — skipping this one."); return "skipped"; }
        let _0x18 = _0x11.config?.application?.id ?? _0x17?.applications?.[0]?.id;
        if ((_0x16 === GoogleTasks.play || _0x16 === GoogleTasks.stream) && !_0x18) { Log.say("Crumb", "Chore had no application identifier — skipping."); return "skipped"; }
      ...
      const _0x2c = async () => {
        ...
        const _0xrefill = () => {
          try {
            ...
            for (const q of _0list) {
              ...
              if (!_0x44c1(q.config, GoogleRoutes.tasks)?.tasks) continue;
        ...
        try {
          _0xwatch = setInterval(() => { if (_0xkill || signal.aborted) { clearInterval(_0xwatch); _0xwatch = null; return; } const p = location.pathname; if (!_0xpaus && p !== _0xroute0) { _0xpaus = true; Log.say("Signpost", "Trail marker moved — holding position."); } else if (_0xpaus && p === _0route0) { _0xpaus = false; Log.say("Signpost", "Back on the trail — resuming."); } }, 2500);
          _0xbe2f(() => { if(_0xwatch) clearInterval(_0xwatch); });
          while (!_0xkill && !signal.aborted) {
            if (!_0xb.length) {
              const _0new = _0xrefill();
              if (_0new > 0) { Log.say("Docket", `${_0new} more chore${_0new === 1 ? "" : "s"} joined the board.`); continue; }
              break;
            }
            ...
            catch (err) { _0xresults.push({ name: _0x2d.config?.messages?.questName ?? "quest", result: "failed" }); Log.say("Crumb", `Stubbed a toe on one chore (moving on): ${err?.message ?? err}`); }
            if (_0xb.length && !_0xkill && !signal.aborted) {
              let _0xgn = false;
              try {
                const _0xnq = _0xb[_0xb.length - 1];
                const _0nt = _0x44c1(_0xnq.config, GoogleRoutes.tasks)?.tasks;
...
while (_0xcc13.size > 0 && !_0xkill && !signal.aborted) await GoogleDelay(1);
          if (_0xkill || signal.aborted) Log.say("Chime", `Last call — shift ended early (${_0xresults.length} activity result${_0xresults.length === 1 ? "" : "s"}).`);
          else if (didWork) Log.say("Garland", `Shelf polished — nothing left (${_0xresults.length} activity result${_0xresults.length === 1 ? "" : "s"}). (Press Alt+Shift+R to flush and restart)`);
        } catch (err) { Log.say("Crumb", `Stubbed a toe: ${err?.message ?? err}`); } 
        finally { _0x1 = _0x2 = _0x3 = _0x4 = _0x5 = _0x6 = _0x7 = _0x8 = _0x9 = _0xb = null; if (!didWork && !_0xkill) GoogleRelease(); }
        if (didWork || _0xkill || signal.aborted) { _0xarmed = true; Log.say("Wreath", (didWork && !_0xkill && !signal.aborted) ? "All polished — press Alt+Shift+R to shake out the rug (refresh), or just keep browsing; nothing moves until you say so." : "Rug's half-shaken — press Alt+Shift+R to finish the job (refresh) whenever you're ready."); } 
        else { GoogleScuttle(); GoogleRelease(); }
      };

      let _0xbootTimer = null;
      const _0xboot = async (ev) => {
        if (ev.origin === location.origin && ev.data === _0xch) { window.removeEventListener("message", _0xboot); clearTimeout(_0xbootTimer); _0xbootTimer = null; Log.say("Cubby", "Note slid under the door — shift started."); await GoogleDelay(2.5 + Math.random() * 5.5); if (!_0xkill && !signal.aborted) _0x2c(); }
      };
      try {
        window.addEventListener("message", _0xboot); _0xbe2f(() => window.removeEventListener("message", _0xboot));
        ...
        _0xbe2f(() => { if (_0xbootTimer) clearTimeout(_0xbootTimer); });
        window.postMessage(_0xch, location.origin);
      } catch (err) { Log.say("Crumb", `Knocked the shelf over setting up: ${err?.message ?? err}`); GoogleScuttle(); GoogleRelease(); }
    } catch (err) { Log.say("Crumb", `Knocked the shelf over setting up: ${err?.message ?? err}`); GoogleScuttle(); GoogleRelease(); }
  })();
