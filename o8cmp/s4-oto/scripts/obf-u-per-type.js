...
//
// Checks per output: syntax, marker count == 1, 佐藤/結衣 contiguous, reversed unlock
// literal 'kcolbUelgooG' present, bidi controls surviving, zero "GoogleUnlock" literal.
'use strict';
const fs = require('fs');
...
const U_SRC = '/home/user/o8cmp/s4-shards/shards/shard-u.js';
const CANON = '/home/user/o8cmp/s4-oto/u/shard-u-out.js';
const RLO = '\u202e', PDF = '\u202c', REV = 'kcolbUelgooG';
...
function verify(p, label) {
  ...
  const checks = {
    ...
    'reversed literal': s.includes(REV),
    'bidi controls': new RegExp('[' + RLO + PDF + ']').test(s),
...
{
  const out = '/home/user/o8cmp/s4-oto/v1-jso-s3matrix/shard-u-out.js';
...
`;
  ...
  const outP = '/home/user/o8cmp/s4-oto/v2-jsc/shard-u-out.js';
  try {
    ...
    // js-confuser unicode-escapes literals (runtime-identical). Restore the marker to a
    // literal, searchable form and the reversed unlock literal's bidi controls to raw chars.
    let out2 = fs.readFileSync(outP, 'utf8');
    out2 = out2.split('\\u4F50\\u85E4 \\u7D50\\u8863').join('佐藤 結衣')
               .split('\\u202EkcolbUelgooG\\u202C').join(RLO + REV + PDF)
               .split('\\u202eKcolbUelgooG\\u202c').join(RLO + REV + PDF);
...
for (const [type, ok] of Object.entries(results)) {
  ...
  const p = `/home/user/o8cmp/s4-oto/${map[type]}/shard-u-out.js`;
