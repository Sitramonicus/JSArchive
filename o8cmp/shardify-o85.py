...
out.append('})();')
final = '\n'.join(out) + '\n'
open('/home/user/o8cmp/O8.5-Shard-2.js', 'w', encoding='utf-8').write(final)
...
# ---------------------------------------------------------------------------
import os
os.makedirs('/home/user/o8cmp/shard-out2', exist_ok=True)
pieces = {
    'a':   wA,   'm': wM,   'n1': wN1,
    'e':   wE,   'n2': wN2, 'aux': wAUX,
}
for tag, wl in pieces.items():
    with open('/home/user/o8cmp/shard-out2/shard-%s.js' % tag, 'w', encoding='utf-8') as f:
        f.write('\n'.join(wl) + '\n')
