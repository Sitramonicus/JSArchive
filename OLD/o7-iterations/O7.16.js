(() => {
    ...
    const inspect = source => {
      ...
      for (const item of entries) {
        ...
        if (guildLike && itemTotal === null && item.members && typeof item.members === "object") {
          const memberSize = item.members instanceof Map ? item.members.size : Object.keys(item.members).length;
          total = (total ?? 0) + memberSize;
        }
  ...
  const _0xact = async (v) => {
    ...
    while (v.cur < v.goal && !_0xkill && Date.now() < _0xactivityDeadline) {
      ...
      const reportedProgress = _0x2b?.body?.progress?.[GoogleTasks.activity]?.value;
      if (reportedProgress !== undefined && reportedProgress !== null && reportedProgress !== "" && typeof reportedProgress !== "boolean" && Number.isFinite(Number(reportedProgress)) && Number(reportedProgress) >= 0) v.cur = Number(reportedProgress);
      firstCoin = false;
      if (++tick % 3 === 1 || GoogleChatter || v.cur >= v.goal) GoogleSay("Abacus", `Random fraction: ${v.cur}/${v.goal}`);
  ...
  const _0x10 = async (_0x11) => {
    ...
    await fn({ q: _0x11, name: _0x11.config?.messages?.questName ?? "that quest", app: _0x18, goal: _0x19, cur: _0x1a, pid: _0x1b, cfgv: _0x11.config?.configVersion, drift: _0xdrift });
    return "processed";
  };
  ...
  const _0x2c = async () => {
    ...
    try {
      _0xwatch = setInterval(() => {
        if (_0xkill && _0xrestores.length) {
          _0xrestores.splice(0).forEach(f => { try { f(); } catch (e) {} });
          _0xpending.clear();
        }
      ...
      if (_0xkill) GoogleSay("Taps", `Last call — shift ended early (${_0xresults.length} activity result${_0xresults.length === 1 ? "" : "s"}).`);
