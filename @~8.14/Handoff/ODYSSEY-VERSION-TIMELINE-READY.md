...
<details>
<summary><strong>Click to expand — the current release (Odyssey 8.13-r3)</strong></summary>

[**O.8.13-r3**](https://gist.githubusercontent.com/Sitramonicus/14aac84f6fc1dded188ea6719bcd4556/raw/e2b9acc3667105825cc021c4f572bf3ee81c5fe5/gistfile1.txt)

</details>

<sup>_Release status:_ O.8.13-r3 remains the maintained and frozen release. O.8.14-B0 is a candidate-only measurement milestone, not a new paste target or shipped revision. CC-02 has not been shipped._</sup>
...
| O.8.5-S2 | ~175 KB | Balanced parts; wording chosen per call from rotating pools; noise inside every part |
| O.8.5-S3 | ~192 KB | Word lists locked at rest; final successful try of obfuscating shards with varied obfuscation |
| O.8.6 | release-scale | Consolidates the packaging line: reproducible assembly, compressed delivery variants, compatibility cleanup, and a more deliberate release boundary |
| O.8.7–O.8.8 | varied | Carrier and delivery experiments: extraction, loader, and reliability work are exercised separately so experimental paths do not silently become the maintained line |
| O.8.9 | release-scale | Runtime string and analysis-resistance pass: protected wording, build-time variation, integrity checks, and controlled fallback behavior become first-class concerns |
| O.8.10 | release-scale | Delivery and lifecycle hardening: the runner, loader, capability checks, cancellation paths, teardown, and per-build identity are brought under repeatable tests |
| O.8.11 | release-scale | Build-pipeline and component-structure revision: part treatment, identifier material, runtime selection, and cross-part compatibility are made more consistent |
| O.8.12 | release-scale | Payload and carrier stabilization: the identifier material, runtime decoding path, fallback presentation, timing behavior, and extraction compatibility receive a coordinated pass |
| O.8.12-r3 / O.8.12-r4 | release-candidate line | Regression and recovery revisions: release checks, failure isolation, carrier compatibility, and operator handoff are tightened before the next maintained line |
| O.8.13 candidate line | release-candidate line | Consolidates the tested runtime, carrier, and packaging work while separating historical candidates from the final release identity |
| O.8.13-r3 | frozen current release | Release freeze: verified runner/bundle/carrier identity, release/archive parity, corrected regression checks, stable fallback behavior, and explicit provenance |
| O.8.14-B0 | candidate-only | Measurement baseline only: evaluator-boundary, host-boundary, branch/lifecycle, allocation, cleanup, and provenance observations; not a shipped script |

<sup>_Note:_ `the naming jumps straight from O.4 to O.7 — the candidate drafts in between were incomplete. O.7.17–O.7.25 were supplied-revision reviews and O.7.32–O.7.37 diagnostics-only steps; their conclusions landed in the listed files. The O.7.42-era concealment experiments were excluded from the maintained line. The maintained line later proceeds through the O.8.6–O.8.13 release families and currently ends at the frozen O.8.13-r3 release._</sup>
...
### O.8.13-r3) — the frozen release (current)

- **Release identity is explicit:** the runner, embedded bundle, carrier, manifests, and archive mirrors are treated as one verifiable release line rather than as interchangeable historical files.
- **The runtime is more uniform across supported host pockets:** capability reads, fallback behavior, and failure handling follow the same guarded shape wherever the environment permits it.
- **The string and identifier layer is less static:** wording, labels, and build material are selected or recovered at runtime, while shipped behavior remains the same for a normal paste.
- **The fallback and honey paths are part of the tested behavior:** altered, unsupported, or suspicious conditions are handled deliberately instead of being allowed to fail into an accidental state.
- **The carrier path is more tolerant and less dependent on one fixed layout:** extraction, prefix handling, and delivery boundaries are covered by compatibility tests.
- **Release verification is part of the product:** provenance, naming, archive parity, malformed-input handling, cleanup, and regression checks are recorded alongside the artifact.
- This is a functional freeze, not a security certification. It still contains dynamic execution and host-task/progress interfaces; it must not be described as network-free or authority-free.

### O.8.13 candidate line) — consolidation before freeze

- Candidate work moves the previously separate runtime, carrier, and packaging experiments onto one reviewable line.
- Generated outputs, compatibility names, and historical candidates are separated from the maintained paste target.
- The acceptance emphasis shifts from “the script appears to work” to repeatable release verification, archive parity, cleanup, and reproducible provenance.

### O.8.12-r3 / O.8.12-r4) — payload and carrier stabilization

- The identifier material is rebalanced so it supplies variation without making the release depend on one conspicuous vocabulary shape.
- Runtime decoding is reduced and made more selective; repeated accesses are handled through a bounded reuse path rather than a uniform decode storm.
- Fallback wording and host-pocket presentation are made less predictable while keeping normal-path behavior stable.
- Carrier work focuses on extraction tolerance, seeded variation, and compatibility with the existing delivery path. Alternative delivery formats remain experiments rather than silent additions to the maintained release.
- The revision line adds recovery notes and operator-facing checks so a failure can be classified as a skip, a fallback, or a release problem.

### O.8.11) — structured build and component revision

- The build pipeline begins treating runtime parts as separately measurable components instead of one undifferentiated text block.
- Identifier generation, per-build variation, cross-part contracts, and output checks are brought into the same repeatable process.
- Compatibility and malformed-input checks are expanded so a component can fail closed without taking down unrelated work.

### O.8.10) — delivery and lifecycle hardening

- The runner and loader receive a stronger lifecycle boundary: startup, cancellation, timeout, normal completion, and failure all have explicit teardown expectations.
- Capability checks become fail-closed, and host-facing operations are kept behind the existing adapter boundary rather than being expanded opportunistically.
- Build identity and compatibility metadata become easier to verify across delivery variants.
- This line establishes the repeatable runner/carrier harness used by later regression work.

### O.8.9) — runtime analysis-resistance line

- Protected strings and identifiers move further toward runtime recovery, with build-time variation and integrity checks used to detect altered execution copies.
- Fallback and honey behavior is made intentional: a changed or unsupported copy should not be mistaken for the normal result.
- The red-team lesson from this line is retained in the release notes: anti-tamper behavior can increase reviewer suspicion, so it is tested and documented rather than treated as proof of security.

### O.8.6–O.8.8) — packaging and carrier transition

- The project moves from a small single-file history into a release process with generated delivery artifacts, extraction checks, compatibility variants, and separate experimental carrier work.
- Reliability work is kept close to the packaging boundary: the loader must either produce the expected result or stop cleanly, and retired experiments are archived instead of being presented as maintained releases.
- These versions establish the packaging and carrier vocabulary used by the later O8.9–O8.13 line.

### O.8.5-S3) — the sealed release
...
- **Shard balancing:** the payload is split into near-equal pieces joined by one tiny shared contract — no single oversized chunk stands out, and no standalone decoy appendix exists.
...
- **Almost-silent default:** the default run logs nothing except the member readout's own occasional line; the verbose logs needs password.
...
- **Logs under lock:** the verbose logs no longer prints freely — it opens only through a salted passphrase check, and the passphrase lives nowhere in the script.
...
- **Graceful skipping:** quests that can't be handled are now stepped over mid-queue instead of jamming the run.
...
- **Renaming:** every readable name replaced by short arbitrary serial-style names.
...
<sup>_Note:_ `letters B–F were never archived — the naming jumps straight from A to G — and the archive also lists two entries both labeled "J," the second of which supersedes the lightweight variant above it. The maintained line later proceeds through the O.8.6–O.8.13 release families and currently ends at the frozen O.8.13-r3 release. O.8.14-B0 is a measurement candidate, not a shipped revision._</sup>
