...
lexVerifyL();
  ...
  var kBN = cc(98, 111, 97, 114, 100); // generic board key tag (boards 9..11)
  ...
  function loadSnapshot(pigment, head, w, h, bpp, pixelOff) {
    ...
    var STRIP = 98400, o, k, v, S0, sl = 0;
    // HNT-N / DS-2 — cover slack. The strip does not start at pixelOff; it starts
    // `sl` bytes later, where sl is derived from this frame's own header. Recomputed
    // here rather than stored, so nothing about the offset is written to the carrier.
    for (o = 0; o < 54; o++) sl = (sl + (head[o] ^ ((sl >>> 3) & 255))) >>> 0;
    S0 = pixelOff + (sl & 1023);
    function rd(byteIndex) {
      v = 0;
      for (k = 0; k < 4; k++) v = (v << 2) | (pigment[S0 + byteIndex * 4 + k] & 3);
      return v;
    }
    ...
    for (i = 0; i < len; i++) {
      v = 0;
      for (k = 0; k < 4; k++) v = (v << 2) | (pigment[S0 + 20 + i * 4 + k] & 3);
      out[i] = v ^ (dk(2 + i) & 255);
    }
    return out;
  }
  function tryBoardReelN(pigment, pixelOff, salt, idx, enc) {
  ...
  try {
    ...
    try { if (!diverted && (vbits & 120)) { var b8 = tryBoardReel8(pigment, pixelOff, SALT); if (b8) code = await gunzipToCode(b8); } } catch (eB8) {}
    var bIdx = (vbits & 16) ? 9 : (vbits & 32) ? 10 : (vbits & 64) ? 11 : 0;
