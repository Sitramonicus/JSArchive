  ...
  const Log = (() => {
    const noop = () => {};
    const _0xq = [];
    const _0xdm = (m, d) => { if (会員 >= 2 && _0xopen) { try { if (d!==undefined) { const _j=JSON.stringify(d); if(_j.length>400) d=JSON.parse(_j.slice(0,400)); } } catch(e){} d!==undefined?console.debug(`[SYS-DIAG] ${m}`, d):console.debug(`[SYS-DIAG] ${m}`); } }; // quota guard
    if (会員 === 0) return { say: noop, diag: noop, warn: noop, info: noop, queue: noop, flush: noop };
    return {
      say: (c, m) => { if (会員 >= 2 && _0xopen) console.debug(`[Google ${c}] ${String(m).slice(0,300)}`); else console.debug(String(m).slice(0,300)); }, // quota guard
      diag: _0xdm,
