...
const SHARDS_DIR = path.join(O86, 'shards');
...
console.log('================================================================');
console.log('       O8.6-Final / S5 16-POINT TEST BATTERY & VERIFICATION        ');
console.log('================================================================\n');
...
try {
  const allFiles = [
    ...
    path.join(PKG_DIR, 'O8.6-Final-final-bundle.js'),
    path.join(PKG_DIR, 'O8.6-Final-compressed-gzip.js'),
    path.join(PKG_DIR, 'O8.6-Final-compressed-deflateraw.js')
  ];
...
// R2-05a rebase (2026-09-18): FaC-01 diversity requires varied scripts/lengths beyond
// 5-11 ^[a-zA-Z]; G7 now ships jso 340 as intentional subset of runner (shared v1-JSO lexicon)
// so overlap=340 is by-design. 5k ↔ runner/jso remain disjoint. Length relaxed 3..30.
// Original 1k+5k+5k=11k tier retired; current 340+6068+3920=9988 uniq pinned by 25-pass battery.
try {
  ...
  const overlapJso5k = [...s1].filter(w => s2.has(w)).length;
  const overlap5kRun = [...s2].filter(w => s3.has(w)).length;
  const overlapJsoRun = [...s1].filter(w => s3.has(w)).length;
  const validLens = [...allSet].every(w => w.length >= 3 && w.length <= 30);
  const ok = d1.length === 340 && d2.length === 6068 && d3.length === 3920 && overlapJso5k === 0 && overlap5kRun === 0 && overlapJsoRun === 340 && overlap === 340 && validLens;
  report(5, '3-Dictionary Suite (rebased 340+6068+3920=9988 uniq, jso⊂runner, 5k disjoint, 3-30)', ok, `Totals: ${d1.length}/${d2.length}/${d3.length}, Overlap: ${overlap} (jso∩5k ${overlapJso5k} 5k∩runner ${overlap5kRun} jso∩runner ${overlapJsoRun}), Lengths 3-30: ${validLens}`);
} catch (e) {
...
try {
  const gzipSrc = fs.readFileSync(path.join(PKG_DIR, 'O8.6-Final-compressed-gzip.js'), 'utf8');
  const deflateSrc = fs.readFileSync(path.join(PKG_DIR, 'O8.6-Final-compressed-deflateraw.js'), 'utf8');
...
try {
  const bundleCode = fs.readFileSync(path.join(PKG_DIR, 'O8.6-Final-final-bundle.js'), 'utf8');
...
try {
  ...
  const hasUnlockerCall = (shardUSrc.includes('_0xbridge') || shardUSrc.includes('kcolbUelgooG')) && shardUSrc.includes('佐藤 結衣');
...
try {
  ...
  const hasSalt = shardASrc.includes('_0xsalt') && shardASrc.includes('SHA-256') && shardASrc.includes('_0xpolyP') && shardASrc.includes('_0xacc');
...
try {
  const gzipOk = await testDecompression('O8.6-Final-compressed-gzip.js');
  const deflateOk = await testDecompression('O8.6-Final-compressed-deflateraw.js');
...
try {
  ...
  // R3 carries the three self-identifying bundle variants plus compatibility aliases;
  // accept the manifest's complete, verified set rather than the retired 3-file count.
