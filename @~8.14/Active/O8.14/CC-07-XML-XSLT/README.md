# CC-07 — XML/XSLT serialized-decoy candidate

**Status:** candidate-only; simulated verification PASS  
**Base:** CC-05 bounded parser candidate  
**FaCs:** FaC-10, FaC-12, FaC-16

This candidate substitutes one bounded serialized xml-xslt record into an existing `shard-n2.js` serialized/no-op budget slot. The slot remains exactly 731 bytes before and after substitution, so the layer is a replacement rather than an appended payload.
...
- `candidate/cc07-xml-xslt-raw-bundle.js` — raw stitched candidate assembly, not a release artifact.
- `reports/build.json` — provenance, representation, and budget-neutrality record.
- `reports/cc07-verification.md` — verification report.
